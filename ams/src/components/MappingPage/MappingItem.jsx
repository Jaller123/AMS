...
const MappingItem = ({
  ...
  useEffect(() => {
    if (autoExpandMappingId && mapping.id === autoExpandMappingId && !expandedMappings[mapping.id]) {
      setExpandedMappings((prev) => ({ ...prev, [mapping.id]: true }));
    }
  }, [autoExpandMappingId, mapping.id, expandedMappings, setExpandedMappings]);
    ...
    <li className={styles.mappingItem} ref={mappingItemRef} data-testid="mapping-item">
      ...
      {expandedMappings[mapping.id] && (
        <>
          ...
          <ResponseEditor
            mappingId={mapping.id}
            relevantResponses={responses.filter((res) => res.reqId === mapping.id)}
