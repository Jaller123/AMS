...
import styles from "./TrafficComponent.module.css";

const TrafficComponent = () => {
  const [trafficData, setTrafficData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState(""); // Filter by URL or Method
  const [error, setError] = useState(null);

  const fetchTrafficData = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://localhost:8080/mappings"); // Replace with your backend endpoint
